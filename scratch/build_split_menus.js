...
const nonVegData = [
  ...
  },
  {
    ...
    items: [
      ...
      { name: "Maestro Special (Bone / Boneless)", price: "219/- | 229/-" },
...
const generateHTML = (data, menuTitle, totalPages) => {
    // We split into multiple pages based on item count to avoid overflow
    // For Veg: fits comfortably in 2 pages
    // For Non-Veg: fits in 2 pages
    let pages = [];
    if (menuTitle === "VEGETARIAN") {
        pages = [
            [data[0], data[1], data[2]], // Soups, Starters, Breads
            [data[3], data[4], data[5], data[6]] // Curries, Biryani, Rice/Noodles, Mojitos
        ];
    } else {
        pages = [
            [data[0], data[1], data[2], data[3], data[4], data[5]], // Soups, Egg Start, Chicken Start, Specials, Seafood Start, Kebabs
            [data[6], data[7], data[8], data[9], data[10], data[11]] // Chicken Curries, Egg/Mutton, Seafood, Biryani, Rice/Noodle, Mojitos
        ];
    }
    ...
    const pagesHtml = pages.map((pageData, index) => {
        ...
        <div class="page">
            <div class="header">
                ...
                <p>R E S T A U R A N T &nbsp;&nbsp;|&nbsp;&nbsp; ${menuTitle}</p>
                <div class="header-line"></div>
            </div>
            <div class="menu-columns">
                ${renderPageContent(pageData)}
            </div>
            <div class="footer">Page ${index + 1} of ${pages.length} | Location: Hyderabad, India | Prices are in INR</div>
        </div>
        `;
    }).join('');
...
<head>
    ...
    <title>Maestro Kitchen ${menuTitle} Menu</title>
    <style>
        :root {
...
fs.writeFileSync('c:/Users/eluri/OneDrive/Desktop/MAESTRO KITCHEN/pos-app/veg_menu.html', generateHTML(vegData, "VEGETARIAN"));
fs.writeFileSync('c:/Users/eluri/OneDrive/Desktop/MAESTRO KITCHEN/pos-app/nonveg_menu.html', generateHTML(nonVegData, "NON-VEGETARIAN"));
console.log('Split menus generated successfully.');
